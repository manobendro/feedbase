...
import { ChevronUp, LayoutGrid, LucideIcon } from 'lucide-react';
import { STATUS_OPTIONS } from '@/lib/constants';
import { FeedbackWithUserProps } from '@/lib/types';

// Card component for draggable feedback items
function PublicFeedbackCard({ feedback }: { feedback: FeedbackWithUserProps }) {
  return (
    <>
...
export default function PublicFeedbackKanban({
  columns,
  data,
  onDataChange,
  sortedBy,
                ...
                {data[label.toLowerCase()]?.map((item) => (
                  <Link key={item.id} href={`/feedback/${item.id}`}>
                    <div
                      ...
                      <PublicFeedbackCard feedback={item} />
                    </div>
                  </Link>
                ))}
              </div>
        ...
        <DragOverlay
          ...
          {activeItem ? (
            <div className='bg-root flex h-fit w-full flex-col gap-3 rounded-md border p-3'>
              <PublicFeedbackCard feedback={activeItem} />
            </div>
          ) : null}
        </DragOverlay>
      </DndContext>
